...
def build_ensemble_mlp(
    ...
    """
    Builds an ensemble of feedforward neural networks.

    Forward:
        x: (B, input_size)
        returns: (n, B, output_size)
    """
    if isinstance(activation, str):
        activation = _str_to_activation[activation]
    if isinstance(output_activation, str):
    ...
    def _build_single() -> nn.Sequential:
        layers = []
        in_size = input_size
        for _ in range(n_layers):
            layers.append(nn.Linear(in_size, size))
            ...
            in_size = size
        layers.append(nn.Linear(in_size, output_size))
        layers.append(output_activation.__class__()) # fresh module
        return nn.Sequential(*layers)

    mlps = nn.ModuleList([_build_single() for _ in range(n)])
    model = EnsembleMLP(mlps)
    model.to(device)
    return model
